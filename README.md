...
The action does not download or bootstrap vcpkg. It requires the runner/toolchain to provide a usable vcpkg installation (preferring `VCPKG_ROOT`, then `VCPKG_INSTALLATION_ROOT`, then `vcpkg` on `PATH`), exports the resolved `VCPKG_ROOT` and `VCPKG_OVERLAY_PORTS`, and configures a credentialed NuGet source for the qigao GitHub Packages feed. If no usable vcpkg installation is present, setup fails immediately; there is no network bootstrap fallback. NuGet network operations use a 600-second timeout because large native packages such as Linux `libpq` can exceed NuGet/vcpkg's 100-second default upload timeout. An explicit `token` input may be supplied for cross-repository package access.
